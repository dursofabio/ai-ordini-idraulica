 ...
 *
 * The spec's real target (p95 < 300ms on 42k product-bases with an active
 * HNSW index) is not reproducible reliably in CI and must be validated in
...
class SearchServicePerformanceTest extends TestCase
{
    ...
    {
        $this->seedProductBases(self::SEED_COUNT);
        ...
        // + 1 main ranked/grouped query. Must not scale with the result count.
    ...
    private function seedProductBases(int $count): void
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            $baseRows[] = [
                'title' => "Scaldabagno modello {$i}",
                'description_ai' => "Scaldabagno a pompa di calore modello {$i} risparmio energetico",
                'grouping_key' => "scaldabagno-modello-{$i}",
                'brand_id' => $brandId,
                'family_id' => $familyId,
        ...
        DB::table('product_bases')->insert($baseRows);

        $baseIds = DB::table('product_bases')->pluck('id');

        $vector = '['.implode(',', array_fill(0, 1024, 0.1)).']';
        $embeddingRows = $baseIds->map(fn ($id) => [
            'product_base_id' => $id,
            'content' => 'seed',
            'model' => 'test-model',
            'dimensions' => 1024,
